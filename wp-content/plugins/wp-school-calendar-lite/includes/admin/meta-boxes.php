...
class WP_School_Calendar_Admin_Meta_Boxes
{
    ...
    public function add_meta_boxes()
    {
        add_meta_box(
            'wpsc-date',
            __( 'Date Settings', 'wp-school-calendar' ),
            array( $this, 'add_date_meta_box' ),
            'important_date',
            'normal',
            'high'
        );
        add_meta_box(
            'wpsc-category',
            __( 'Category', 'wp-school-calendar' ),
            array( $this, 'add_category_meta_box' ),
            'important_date',
            'normal',
            'high'
        );
    ...
    public function add_date_meta_box( $post, $box )
    {
        ...
        echo  esc_html__( 'Start Date', 'wp-school-calendar' ) ;
        ?></th>
				<td class="forminp">
                    <input type="text" id="start-datepicker" value="" readonly="readonly">
                    <input type="hidden" id="start-datepicker-alt" name="start_date" value="<?php 
        ...
        ?>">
            ...
            <tr valign="top">
				<th scope="row" class="titledesc"><?php 
        echo  esc_html__( 'End Date', 'wp-school-calendar' ) ;
        ?></th>
				<td class="forminp">
                    <input type="text" id="end-datepicker" value="" readonly="readonly">
                    <input type="hidden" id="end-datepicker-alt" name="end_date" value="<?php 
        ...
        ?>">
            ...
            <tr valign="top">
				<th scope="row" class="titledesc"><?php 
        echo  esc_html__( 'Weekend Options', 'wp-school-calendar' ) ;
        ?></th>
				<td class="forminp">
                    <label><input type="checkbox" name="exclude_weekend" value="Y"<?php 
        checked( 'Y', $exclude_weekend );
        ?>> <?php 
        echo  __( 'Exclude weekends from this date range.', 'wp-school-calendar' ) ;
        ?></label>
				</td>
			</tr>
        </table>
    ...
    public function add_category_meta_box( $post, $box )
    {
        ...
        <table class="form-table">
            <tr valign="top">
                <th scope="row" class="titledesc"><?php 
        echo  esc_html__( 'Select Category', 'wp-school-calendar' ) ;
        ?></th>
                <td class="forminp">
        ...
        foreach ( $category_options as $option ) {
            ?>
                        <option value="<?php 
            echo  $option['category_id'] ;
            ?>"<?php 
            echo  selected( $option['category_id'], $category_id ) ;
            ?>><?php 
            echo  $option['name'] ;
            ?></option>
                        <?php 
...
WP_School_Calendar_Admin_Meta_Boxes::instance();